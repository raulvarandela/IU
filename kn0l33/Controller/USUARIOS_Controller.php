...
function get_data_form() {
    ...
    if($_REQUEST['action']=='ADD' || $_REQUEST['action']=='EDIT'){ //saber que opción es añadir o editar
        $foto = $_FILES['fotopersonal']; //la foto es el archivo
    }
    else{ //si no, es buscar o borrar
        $foto = $_POST['fotopersonal']; //foto la trato como un string
    }

    $user = new USUARIOS_Model($_POST['login'], $_POST['password'], $_POST['DNI'], $_POST['nombre'],$_POST['apellidos'], $_POST['telefono'], $_POST['email'], $_POST['FechaNacimiento'], $foto, $_POST['sexo']);//Se crea un usuario
		...
		Switch ($_REQUEST['action']){
			...
			case 'SEARCH':
				if (!$_POST){
					...
					$lista = array('login','nombre','apellidos','email'); //para que me muestre la info reducida
			...
			default://caso por defecto,se muestran todas las tuplas
				...
				$lista = array('login','nombre','apellidos','email');//los atributos a mostrar en el showall
