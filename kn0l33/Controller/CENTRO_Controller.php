<?php
//Archivo : CENTRO_Controller
//Creado el : 10/10/2019
//Creado por: kn0l33
//Función: controla la gestion de los centros
//-------------------------------------------------------
...
// la función get_data_form() recoge los valores que vienen del formulario por medio de post y la action a realizar, crea una instancia CENTRO y la devuelve
	...
	if (!isset($_POST['NOMBRECENTRO'])){//compuebo si llega por el metodo POST
        $_POST['NOMBRECENTRO'] = '';
    }
	if (!isset($_POST['DIRECCIONCENTRO'])){//compuebo si llega por el metodo POST
        $_POST['DIRECCIONCENTRO'] = '';
    }
    if (!isset($_POST['RESPONSABLECENTRO'])){//compuebo si llega por el metodo POST
		...
		Switch ($_REQUEST['action']){
			...
			case 'SEARCH':
				if (!$_POST){//nos llega el centro al search por get
					...
					new CENTRO_SHOWALL($lista, $datos, '../index.php');//invoco a la vista de showall
				}
				break;
			case 'SHOWCURRENT':
