<?php
//Archivo : EDIFICIO_Controller
//Creado el : 10/10/2019
//Creado por: kn0l33
//Función: controla la gestion de edificios
//-------------------------------------------------------
...
// la función get_data_form() recoge los valores que vienen del formulario por medio de post y la action a realizar, crea una instancia edificio y la devuelve
	...
	if (!isset($_POST['CODEDIFICIO'])){//compuebo si llega por el metodo POST
        $_POST['CODEDIFICIO'] = '';
    }
	if (!isset($_POST['NOMBREEDIFICIO'])){//compuebo si llega por el metodo POST
        $_POST['NOMBREEDIFICIO'] = '';
    }
	if (!isset($_POST['DIRECCIONEDIFICIO'])){//compuebo si llega por el metodo POST
        $_POST['DIRECCIONEDIFICIO'] = '';
    }
    if (!isset($_POST['CAMPUSEDIFICIO'])){//compuebo si llega por el metodo POST
		...
		Switch ($_REQUEST['action']){
			...
			case 'SEARCH':
				if (!$_POST){//nos llega el edificio a search por get
					...
					$lista = array('CODEDIFICIO','NOMBREEDIFICIO','CAMPUSEDIFICIO');//le paso una parte de los atributos como resultado
			...
			default://caso por defecto, muestro todas las tuplas
				...
				$lista = array('CODEDIFICIO','NOMBREEDIFICIO','CAMPUSEDIFICIO');//le paso una parte de los atributos para mostar
