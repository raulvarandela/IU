...
// la función get_data_form() recoge los valores que vienen del formulario por medio de post y la action a realizar, crea una instancia espacio y la devuelve
	...
	if (!isset($_POST['CODCENTRO'])){//compuebo si llega por el metodo POST
        $_POST['CODCENTRO'] = '';
    }
	if (!isset($_POST['TIPO'])){//compuebo si llega por el metodo POST
        $_POST['TIPO'] = '';
    }
    if (!isset($_POST['SUPERFICIEESPACIO'])){//compuebo si llega por el metodo POST
        $_POST['SUPERFICIEESPACIO'] = '';
    }
    if (!isset($_POST['NUMINVENTARIOESPACIO'])){//compuebo si llega por el metodo POST
		...
		Switch ($_REQUEST['action']){
			...
			case 'SEARCH':
				if (!$_POST){//nos llega el ESPACIO a  search por get
					...
					$lista = array('CODESPACIO','TIPO','SUPERFICIEESPACIO');//le paso una parte de los atributos como resultado
			...
			default://caso por defecto, muestro todas las tuplas
				...
				$lista = array('CODESPACIO','TIPO','SUPERFICIEESPACIO');//le paso una parte de los atributos para mostar
